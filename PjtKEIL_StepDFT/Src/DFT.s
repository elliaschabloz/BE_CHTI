...
DFT proc
	...
	MOV r4, r2 ;r2 va dans r4
	MOV r2, #0	;index
	

	MOV r6, #0 ;somme
boucle	
	LDRH r12,[r0,r2,LSL #1] ; <== x(n) au format 1.12 ou 4.12 avec 3 zéros au début
	;LDR r4, =TabCos
	
	MUL r5, r1, r2 ; k*n
	AND r5, r5, #63 ;mod 64
	LDRSH r5, [r4,r5,LSL #1] ; <== cos(kn) au format 1.15
	MUL r5, r12, r5 ; <== x(n)*cos(kn) au format 5.27
	ASR r5, r5, #14 ; <== x(n)*cos(kn) au format 5.13
	ADD r6, r5 ; <== Somme des éléments en 11.13 
	...
	ASR r6, r6, #10 ; format 11.3 (on a pris 3 bits de sécurité)
	MOV r0, r6
	MUL r0,r0 ;format 22.6
